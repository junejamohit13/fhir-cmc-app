...
import { createEnhancedTest, fetchProtocols } from '../services/api';

function CreateEnhancedTest() {
  ...
  const handleSubmit = async (testData) => {
    try {
      ...
      successMessage.innerHTML = 'Creating enhanced test... This may take a few moments.';
      ...
      console.log('Submitting enhanced test data:', testData);
      const response = await createEnhancedTest(testData);
      console.log('Enhanced test created with ID:', response.test_id);
      ...
      setError('Failed to create enhanced test. Please try again.');
      setSubmitting(false);
    }
  };
      ...
      <Paper sx={{ p: 3, mb: 4 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
          <Typography variant="h4" component="h1" gutterBottom>
            Create Enhanced Stability Test
          </Typography>
